...
void renderToImage(size_t width, size_t height, size_t numComponents, unsigned char *outPixels, std::function<void()> drawScene) {
    ...
    glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, textureObject, 0);
    ...
    if (currentlyBoundFBO != framebufferObject) {
        ...
        std::clog << "Warning: renderToImage - GL_DRAW_FRAMEBUFFER_BINDING has changed during drawScene. It might lead to unexpected behavior." << std::endl;
    }

    glBindTexture(GL_TEXTURE_2D, textureObject);
    glGetTexImage(GL_TEXTURE_2D, 0, numComponents == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE, outPixels);
